  ...
  if (theuser)
    return (
      <div key={theuser._id} className="user-card">
        <div>
          ...
          {theuser.image_link && (
            <img
              ...
              src={`${baseUrl}/${theuser.image_link}`}
              alt="profile picture"
            />
          )}
          <section>
            ...
            {showAdditionnalInfo &&
              (showAdditionnalInfo & ShowAdditionnalInfo.Channel) !== 0 && (
                <div>
                  <span>Channels</span>
                  <ul>
                    {(user?.role == UserRole.admin ||
                      user?._id == theuser._id) &&
                      theuser.group_chat_list_ids.length > 0 &&
                      theuser.group_chat_list_ids.map((chat: Channel) => (
                        <ShowChat
                          key={chat._id}
                          channel={chat}
                          user={theuser}
                          ...
                          onManage={(id) =>
                            navigate(`${Route.manageChannels}/${id}`)
                          }
                          buttonShow={
                            ShowChatButton.Chat | ShowChatButton.Manage
                          }
                        />
                      ))}
                  </ul>
            ...
            {showAdditionnalInfo &&
              (showAdditionnalInfo & ShowAdditionnalInfo.CommonChannel) !==
                0 && (
                <div>
                  <span>Common Channels</span>
                  <ul>
                    {theuser.hasOwnProperty("common_channels") &&
                      theuser.common_channels!.length > 0 &&
                      theuser.common_channels!.map((chat: Channel) => (
                        <ShowChat
                          key={chat._id}
                          channel={chat}
                          user={theuser}
            ...
            {showAdditionnalInfo &&
              (showAdditionnalInfo & ShowAdditionnalInfo.CommonActivity) !==
                ...
                <div>
                  ...
                  <ul>
                    {theuser.hasOwnProperty("common_activity") &&
                      theuser.common_activity!.length > 0 &&
                      theuser.common_activity!.map((act: Activity) => (
                        <ShowActivity
                          key={act._id}
                          activity={act}
                          user={theuser}
                          onManage={() =>
                            navigate(`${Route.manageActivity}/${act._id}`)
                          }
                          buttonShow={
                            ShowActivityButton.Activity |
                            ShowActivityButton.Manage
                          }
                        />
                      ))}
                  </ul>
        ...
        <div>
          {(buttonShow & ShowUserButton.ViewUser) !== 0 && onViewUser && (
            <button id={theuser._id} onClick={() => onViewUser(theuser._id)}>
              Voir l'utilisateur
            </button>
          )}
          ...
          {(buttonShow & ShowUserButton.Delete) !== 0 && onDelete && (
