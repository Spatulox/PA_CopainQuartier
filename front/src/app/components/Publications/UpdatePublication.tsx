    ...
    <div key={publication._id} className="update-publication-form">
      {err && err.length > 0 && (
        <div className="error-messages">
          {err && err.length > 0 && err.map((e: any) => <p>{e}</p>)}
        </div>
      )}
      <h2>
        <input
      ...
      <div>
        <span>
          <button
            onClick={() => navigate(`${Route.user}/${publication.author?._id}`)}
          >
            {publication.author?.email || publication.author?._id == user?._id
              ? user?.email
              : "Unknown"}
          </button>
        </span>
      ...
      <div>
        {publication.activity != null ? (
          <button
            onClick={() =>
              navigate(`${Route.activity}/${publication.activity?._id}`)
            }
          >
            Voir l'activité
          </button>
        ...
        <button onClick={() => onDelete(publication._id)}>Supprimer </button>
      </div>
    </div>
  );
}
